...
export const Header = ({
  ...
  const loadSuggestions = async () => {
    const {
      data
    } = await supabase.from("products").select("id, name, image_url").eq("active", true).ilike("name", `%${searchQuery}%`).limit(5);
    if (data) {
      setSuggestions(data);
  ...
  const loadCategories = async () => {
    const {
      data
    } = await supabase.from("categories").select("id, name, parent_id").order("name");
        ...
        <div className="container mx-auto px-4 py-2 md:py-3">
          <div className="flex items-center justify-between gap-2 md:gap-6">
                ...
                <div className="flex flex-col gap-4 mt-6">
                  ...
                  <div className="space-y-2">
                    ...
                    return <div key={category.id} className="space-y-1">
                          <Link to={`/produtos?categoria=${category.id}`} className="block py-2 font-medium hover:text-primary transition-colors" onClick={() => setMobileMenuOpen(false)}>
                            {category.name}
                          </Link>
                          {subcategories.length > 0 && <div className="pl-4 space-y-1">
                              {subcategories.map(sub => <Link key={sub.id} to={`/produtos?categoria=${sub.id}`} className="block py-1.5 text-sm text-muted-foreground hover:text-primary transition-colors" onClick={() => setMobileMenuOpen(false)}>
                                  {sub.name}
                                </Link>)}
                            </div>}
                        </div>;
            ...
            <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-2xl relative" ref={searchRef}>
              ...
              {showSuggestions && suggestions.length > 0 && <div className="absolute top-full mt-2 w-full bg-background border border-border rounded-xl shadow-elegant z-[150] max-h-96 overflow-y-auto">
                  {suggestions.map(product => <button key={product.id} onClick={() => handleSuggestionClick(product.id)} className="w-full px-4 py-3 flex items-center gap-3 hover:bg-muted transition-colors text-left border-b border-border/50 last:border-0">
                      {product.image_url ? <img src={product.image_url} alt={product.name} className="w-12 h-12 object-cover rounded-lg flex-shrink-0" /> : <div className="w-12 h-12 bg-muted rounded-lg flex items-center justify-center flex-shrink-0">
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
              ...
              <Link to="/carrinho">
                <Button variant="ghost" size="icon" className="relative hover:bg-muted rounded-xl transition-all h-10 w-10 md:h-11 md:w-11">
                  ...
                  {cartItemsCount > 0 && <span className="absolute -top-1 -right-1 bg-gradient-accent text-accent-foreground rounded-full w-5 h-5 text-xs flex items-center justify-center font-bold shadow-md">
                      {cartItemsCount}
                    </span>}
                </Button>
              </Link>
          ...
          <div className="flex items-center gap-1 overflow-x-auto py-2 scrollbar-hide">
            <DropdownMenu open={openDropdown === 'all'} onOpenChange={open => setOpenDropdown(open ? 'all' : null)}>
              ...
              <DropdownMenuContent align="start" className="w-72 bg-popover z-[100] shadow-elegant rounded-xl border-border/50 max-h-[70vh] overflow-y-auto" onMouseEnter={() => setOpenDropdown('all')} onMouseLeave={() => setOpenDropdown(null)}>
                {getMainCategories().map(category => <DropdownMenuItem key={category.id} asChild>
            ...
            if (subcategories.length > 0) {
              return <DropdownMenu key={category.id} open={openDropdown === category.id} onOpenChange={open => setOpenDropdown(open ? category.id : null)}>
                    ...
                    <DropdownMenuContent className="bg-popover z-[100] shadow-elegant rounded-xl border-border/50 w-56" onMouseEnter={() => setOpenDropdown(category.id)} onMouseLeave={() => setOpenDropdown(null)}>
                      ...
                      {subcategories.map(sub => <DropdownMenuItem key={sub.id} asChild>
